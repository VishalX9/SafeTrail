...
const TravelerConnect = () => {
  ...
  const handleCreateGroup = (e: React.FormEvent) => {
    e.preventDefault();
    alert(`🎉 Travel Group Created!\n\nTitle: ${newGroupData.title}\nDestination: ${newGroupData.destination}\nDate: ${newGroupData.date}\nCost: ₹${newGroupData.cost}\n\nYour group is now live and other travelers can join! Group ID: TG${Date.now()}`);
    setNewGroupData({ title: "", destination: "", date: "", maxMembers: "", cost: "", description: "" });
  };

  const handleJoinGroup = (group: TravelGroup) => {
    alert(`✅ Join Request Sent!\n\nGroup: ${group.title}\nDestination: ${group.destination}\nOrganizer: ${group.organizer}\n\nThe organizer will contact you soon with details. Check your profile for updates!`);
  };

  const handleCreatePost = () => {
    if (!newPostContent.trim()) return;
    alert(`📝 Travel Experience Posted!\n\nYour post about ${selectedDestination || "North East India"} has been shared with the community.\n\nOther travelers can now see your experience and ask questions!`);
    setNewPostContent("");
