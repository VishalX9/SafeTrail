...
export function LostAndFound() {
  ...
  const handleSubmitItem = (e: React.FormEvent) => {
    ...
    alert(`✅ ${newItem.type === 'lost' ? 'Lost' : 'Found'} Item Reported!\n\nTitle: ${newItem.title}\nLocation: ${newItem.location}\n\nYour report is now live and other travelers can see it. You'll be notified if someone responds.\n\nReport ID: LF${Date.now()}`);
  };

  const handleContactOwner = (item: LostFoundItem) => {
    alert(`📞 Contact Information\n\n${item.type === 'lost' ? 'Lost by' : 'Found by'}: ${item.reportedBy}\nPhone: ${item.contactInfo}\nLocation: ${item.location}\nDate: ${item.date}\n\n${item.type === 'lost' ? 'Please call if you have found this item.' : 'Please call to claim this item with proper identification.'}`);
  };
