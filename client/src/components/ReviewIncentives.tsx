...
export function ReviewIncentives() {
  ...
  const handleSubmitReview = (e: React.FormEvent) => {
    ...
    if (!newReview.location || !newReview.content) {
      alert("Please fill in location and review content");
      return;
    }
    ...
    // Show incentive popup
    alert(`🎉 Review Submitted Successfully!\n\n✨ Points Earned: ${pointsEarned}\n💰 Total Points: ${userPoints + pointsEarned}\n🏆 Level: ${userLevel}\n\nThank you for helping fellow travelers! Your review helps others make informed decisions.`);
  };

  const handleClaimReward = (reward: Reward) => {
    if (userPoints >= reward.pointsRequired && reward.available) {
      setUserPoints(prev => prev - reward.pointsRequired);
      alert(`🎁 Reward Claimed!\n\n${reward.icon} ${reward.title}\n\n${reward.description}\n\nPoints Used: ${reward.pointsRequired}\nRemaining Points: ${userPoints - reward.pointsRequired}\n\nReward details will be sent to your registered email.`);
    } else if (!reward.available) {
      alert("This reward is currently unavailable. Please check back later!");
    } else {
      alert(`Insufficient points! You need ${reward.pointsRequired - userPoints} more points to claim this reward.`);
    }
  };
